                    ...
                    match headers_val {
                        Value::Object(obj_rc) => {
                            ...
                            for (key, value) in obj.iter() {
                                // Security check: Do not allow overriding restricted headers
                                let lower_key = key.to_lowercase();
                                if lower_key == "content-type" || lower_key == "content-length" {
                                    eprintln!(
                                        "Warning: Ignoring restricted header '{}' in respond statement",
                                        key
                                    );
                                    continue;
                                }
                                ...
                                let value_str = match value {
                                    ...
                                    _ => {
                                        return Err(RuntimeError::new(
                                            format!(
                                                "Header value for '{}' must be Text, Number, or Boolean, got {}",
                                                key,
                                                value.type_name()
                                            ),
                                            *line,
                        ...
                        _ => {
                            return Err(RuntimeError::new(
                                format!(
                                    "Headers must be a map/object, got {}",
