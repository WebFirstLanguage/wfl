...
pub fn register_time(env: &mut Environment) {
    let _ = env.define("today", Value::new_native_function("today", native_today));
    let _ = env.define("now", Value::new_native_function("now", native_now));
    let _ = env.define(
        "datetime_now",
        Value::new_native_function("datetime_now", native_datetime_now),
    );
    let _ = env.define(
        "format_date",
        Value::new_native_function("format_date", native_format_date),
    );
    let _ = env.define(
        "format_time",
        Value::new_native_function("format_time", native_format_time),
    );
    let _ = env.define(
        "format_datetime",
        Value::new_native_function("format_datetime", native_format_datetime),
    );
    let _ = env.define(
        "parse_date",
        Value::new_native_function("parse_date", native_parse_date),
    );
    let _ = env.define(
        "parse_time",
        Value::new_native_function("parse_time", native_parse_time),
    );
    let _ = env.define(
        "create_time",
        Value::new_native_function("create_time", native_create_time),
    );
    let _ = env.define(
        "create_date",
        Value::new_native_function("create_date", native_create_date),
    );
    let _ = env.define(
        "add_days",
        Value::new_native_function("add_days", native_add_days),
    );
    let _ = env.define(
        "days_between",
        Value::new_native_function("days_between", native_days_between),
    );
    let _ = env.define(
        "current_date",
        Value::new_native_function("current_date", native_current_date),
    );
}
