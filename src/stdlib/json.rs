...
pub fn register_json(env: &mut Environment) {
    let _ = env.define_native("parse_json", native_parse_json);
    let _ = env.define_native("stringify_json", native_stringify_json);
    let _ = env.define_native("stringify_json_pretty", native_stringify_json_pretty);
}
