...
pub fn register_crypto(env: &mut Environment) {
    let _ = env.define_native("wflhash256", native_wflhash256);
    let _ = env.define_native("wflhash512", native_wflhash512);
    let _ = env.define_native("wflhash256_with_salt", native_wflhash256_with_salt);
    let _ = env.define_native("wflmac256", native_wflmac256);
    let _ = env.define_native("generate_csrf_token", native_generate_csrf_token);
}
