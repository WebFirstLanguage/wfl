...
pub fn register_list(env: &mut Environment) {
    env.define_native("length", native_length);
    env.define_native("push", native_push);
    env.define_native("pop", native_pop);
    env.define_native("contains", native_contains);
    env.define_native("indexof", native_indexof);
    env.define_native("index_of", native_indexof);

    // Batch 3: Basic List Manipulation
    env.define_native("shift", native_shift);
    env.define_native("unshift", native_unshift);
    env.define_native("remove_at", native_remove_at);
    env.define_native("removeat", native_remove_at);
    env.define_native("insert_at", native_insert_at);
    env.define_native("insertat", native_insert_at);
    env.define_native("clear", native_clear);
    env.define_native("slice", native_slice);
    env.define_native("concat", native_concat);
    env.define_native("includes", native_contains);

    // Batch 4: List Utilities
    env.define_native("join", native_join);
    env.define_native("unique", native_unique);
    env.define_native("count", native_count);
    env.define_native("size", native_length);
    env.define_native("fill", native_fill);

    // Batch 5: Sort & Reverse
    env.define_native("sort", native_sort);
    env.define_native("reverse_list", native_reverse_list);

    // Batch 6: List Search
    env.define_native("find", native_find);
    env.define_native("find_index", native_indexof);
    env.define_native("every", native_every);
    env.define_native("some", native_some);
}
