...
fn percent_decode(s: &str) -> Cow<'_, str> {
    ...
    // Fast path: if there are no '+' or '%' characters, return the string as-is
    // without allocations. This speeds up parse_key_value_pairs by avoiding
    // unnecessary Vec and String allocations on unencoded strings.
...
fn parse_key_value_pairs(
    ...
    for pair in input.split(delimiter) {
        ...
        if let Some((key, value)) = pair.split_once('=') {
            ...
            let val = match decoded_value {
                Cow::Owned(s) => Arc::from(s),
                Cow::Borrowed(s) => Arc::from(s),
            };

            params.insert(decoded_key.into_owned(), Value::Text(val));
