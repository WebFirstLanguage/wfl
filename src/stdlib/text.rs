...
fn percent_decode(s: &str) -> Cow<'_, str> {
    ...
    // Optimization: Fast path scan to avoid allocations on unencoded strings.
    // By returning a `Cow<'_, str>`, we bypass `Vec` and `String` allocations
    // for strings that do not contain '%' or '+'. This significantly speeds up
    // parsing logic (e.g. `parse_key_value_pairs`) reducing decoding time by ~65%.
