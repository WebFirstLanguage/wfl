...
pub fn native_substring(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    // Optimization: Return a clone of the original Arc<str> to avoid allocation if the requested substring covers the entire original string.
...
pub fn native_replace(args: Vec<Value>) -> Result<Value, RuntimeError> {
    ...
    // Optimization: Return a clone of the original Arc<str> to avoid allocation if the text does not contain the old string.
...
pub fn native_reverse_text(args: Vec<Value>) -> Result<Value, RuntimeError> {
    unary_text_op("reverse", args, |text| {
        // Optimization: Avoid intermediate String allocation from collect()
        // since the iterator's size_hint() cannot determine the exact byte capacity
        // needed for variable-width chars, causing intermediate re-allocations.
        // Pre-allocate using the byte length of the original string.
        let mut result = String::with_capacity(text.len());
