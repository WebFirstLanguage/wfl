...
fn phc(cost: &str) -> String {
    ...
    format!("$argon2id$v=19${cost}$c29tZXNhbHQxMjM0NTY3OA$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
}
...
fn policy_revalidated_after_mutation_and_never_leaks_password_in_errors() {
    ...
    if let Value::Object(fields) = &candidate {
        fields.borrow_mut().insert("memory_kib".into(), Value::Number(1.0));
    }
    let error = call("hash_password_with_policy", vec![text("private-value"), candidate])
...
async fn policy_hash_is_salted_argon2id_with_exact_costs_and_roundtrips() {
    ...
    assert!(matches!(call("verify_password", vec![text("maintain-me"), text(&first)]), Ok(Value::Bool(true))));
    assert!(matches!(call("verify_password", vec![text("wrong"), text(&first)]), Ok(Value::Bool(false))));
    assert!(matches!(needs_rehash(&first, args[1].clone()), Ok(Value::Bool(false))));
}

#[test]
fn rehash_upgrades_weak_hashes_and_never_silently_lowers_other_costs() {
    let desired = policy(32768.0, 3.0, 1.0);
    assert!(matches!(needs_rehash(&phc("m=19456,t=2,p=1"), desired.clone()), Ok(Value::Bool(true))));
    assert!(matches!(needs_rehash(&phc("m=32768,t=3,p=1"), desired.clone()), Ok(Value::Bool(false))));
    assert!(matches!(needs_rehash(&phc("m=65536,t=4,p=2"), desired.clone()), Ok(Value::Bool(false))));
...
fn rehash_checks_algorithm_version_salt_and_output() {
    ...
    for old in [
        ...
        stored.replace("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA", "AAAAAAAAAAAAAAAAAAAAAA"),
    ] {
        assert!(matches!(needs_rehash(&old, desired.clone()), Ok(Value::Bool(true))), "did not identify legacy parameters: {old}");
    }
}
...
fn rehash_rejects_missing_unknown_malformed_and_oversized_metadata() {
    ...
    for stored in [
        ...
        assert!(needs_rehash(&stored, desired.clone()).is_err(), "accepted malformed stored hash");
    }
}
...
async fn policy_hash_leaves_interpreter_thread_available_and_validates_input() {
    ...
    route("hash_password_with_policy", &args).expect("hash routes").await.unwrap();
    assert!(progressed.load(Ordering::SeqCst));
    sibling.await.unwrap();

    let long_args = [text(&"x".repeat(4097)), args[1].clone()];
    assert!(route("hash_password_with_policy", &long_args).unwrap().await.is_err());
    assert!(route("hash_password_with_policy", &[]).unwrap().await.is_err());
...
async fn password_policy_is_available_in_wfl_and_default_hashing_stays_compatible() {
    assert!(common::expect_bool_result(common::run_wfl_code(r#"
    ...
    assert!(matches!(needs_rehash(&original, policy(19456.0, 2.0, 1.0)), Ok(Value::Bool(false))));
    assert!(call("hash_password", vec![text("compatible"), policy(19456.0, 2.0, 1.0)]).is_err());
}
