...
fn store(capacity: f64) -> Value {
    call(
        ...
        vec![num(3600.0), num(900.0), num(capacity)],
    )
    .unwrap()
}
...
fn sessions_are_isolated_and_unforgeable() {
    ...
    for token in [&id, &csrf] {
        let Value::Text(token) = token else {
            panic!("token must be text")
        };
    ...
    let Value::Object(record) = first else {
        unreachable!()
    };
    ...
    let nested = Value::List(Rc::new(RefCell::new(vec![store_a.clone()])));
    assert_eq!(nested, nested.deep_clone());
    assert!(call("stringify_json", vec![store_a]).is_err());
}
...
fn rotation_and_revocation_invalidate_prior_credentials() {
    ...
    assert_eq!(
        call("session_rotate", vec![store.clone(), old_id]).unwrap(),
        Value::Nothing
    );
    assert_eq!(
        call("session_revoke", vec![store.clone(), fresh_id.clone()]).unwrap(),
        Value::Bool(true)
    );
    assert_eq!(lookup(&store, &fresh_id), Value::Nothing);
    assert_eq!(
        call("session_revoke", vec![store, fresh_id]).unwrap(),
        Value::Bool(false)
    );
}
...
fn account_revocation_and_capacity_do_not_evict_other_accounts() {
    ...
    assert_eq!(
        call("session_revoke_account", vec![store.clone(), text("alice")]).unwrap(),
        num(2.0)
    );
...
fn request(method: &str, cookie: &str, csrf: &str) -> Value {
    let headers = Value::Object(Rc::new(RefCell::new(HashMap::from([
        ("cookie".into(), text(cookie)),
        ("x-csrf-token".into(), text(csrf)),
    ]))));
    Value::Object(Rc::new(RefCell::new(HashMap::from([
        ("method".into(), text(method)),
...
fn csrf_guard_requires_bound_session_and_rejects_ambiguous_cookies() {
    ...
    let guard = |method: &str, cookie: &str, token: &str| {
        call(
            "session_csrf_guard",
            vec![store.clone(), request(method, cookie, token)],
        )
        .unwrap()
    };
    ...
    assert_eq!(
        guard("POST", &cookie, &field(&second, "csrf_token").to_string()),
        Value::Bool(false)
    );
    assert_eq!(
        guard("POST", &format!("{cookie}; {cookie}"), &csrf),
        Value::Bool(false)
    );
    assert_eq!(guard("GET", "", ""), Value::Bool(false));
    assert_eq!(
        guard("POST", &cookie, &"a".repeat(8192)),
        Value::Bool(false)
    );
    assert_eq!(
        call("session_csrf_guard", vec![store.clone(), Value::Nothing]).unwrap(),
        Value::Bool(false)
    );
...
fn cookies_have_secure_defaults_and_reject_header_injection() {
    ...
    let cookie = call("session_cookie", vec![field(&session, "id")])
        .unwrap()
        .to_string();
...
fn account_limiter_counts_attempts_and_fails_closed_when_full() {
    let limiter = call(
        ...
        vec![num(2.0), num(60.0), num(2.0)],
    )
    .unwrap();
    let allow =
        |key: &str| call("account_rate_limit_allow", vec![limiter.clone(), text(key)]).unwrap();
...
fn auth_policy_rejects_invalid_and_oversized_input() {
    ...
    assert!(
        call(
            "session_create",
            vec![store.clone(), text(&"a".repeat(1025))]
        )
        .is_err()
    );
...
fn expired_sessions_and_limiter_windows_reclaim_capacity() {
    ...
    assert_eq!(
        call("session_rotate", vec![store.clone(), field(&first, "id")]).unwrap(),
        Value::Nothing
    );
    session(&store, "bob");
    assert_eq!(
        call("account_rate_limit_allow", vec![limiter, text("bob")]).unwrap(),
        Value::Bool(true)
    );
}
