...
## 2025-03-07 - text parsing optimizations
**Learning:** `text.replace` and `text.split` are already heavily optimized in the Rust standard library. Trying to avoid their allocations via a preliminary `.contains()` check actually regresses performance for hits because it results in a double scan of the string without significantly improving misses. The `percent_decode` operation, however, manually constructs a `Vec<u8>` for every string, so a quick fast-path scan for `%` and `+` and returning `Cow::Borrowed` offers a massive ~50% performance improvement for unencoded queries.
**Action:** When optimizing standard Rust string operations, avoid adding extra `.contains()` checks unless misses are overwhelmingly more common. Focus instead on operations that do manual, unconditional heap allocations (like `percent_decode`). Use `Cow` to avoid intermediate `Vec` and `String` allocations, and use heuristics for `HashMap` pre-allocation (`capacity = len / 4 + 1`) to avoid reallocations.
