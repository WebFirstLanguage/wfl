...
## 2026-03-12 - [Optimize percent decoding and string parsing allocation]
**Learning:** Returning an owned `String` from decoding functions like `percent_decode` incurs a high allocation cost when the input requires no decoding. Similarly, pre-allocating `HashMap`s with heuristic sizing prevents constant reallocations during parses, like in `parse_key_value_pairs`. Checking for characters to decode twice using multiple `.contains()` iterations is slower than a single pass with `.iter().any()`.
**Action:** Use `Cow<'_, str>` to allow returning a borrowed string in the fast path where no decoding is needed. Pre-allocate HashMaps when rough target sizes are predictable, such as  for URL-encoded pairs. Use `.iter().any(|&b| ...)` over multiple `.contains()` calls.

## 2026-03-05 - [Optimize percent decoding and string parsing allocation]
**Learning:** Returning an owned `String` from decoding functions like `percent_decode` incurs a high allocation cost when the input requires no decoding. Similarly, pre-allocating `HashMap`s with heuristic sizing prevents constant reallocations during parses, like in `parse_key_value_pairs`. Checking for characters to decode twice using multiple `.contains()` iterations is slower than a single pass with `.iter().any()`.
**Action:** Use `Cow<'_, str>` to allow returning a borrowed string in the fast path where no decoding is needed. Pre-allocate HashMaps when rough target sizes are predictable, such as `HashMap::with_capacity((input.len() / 4) + 1)` for URL-encoded pairs. Use `.iter().any(|&b| ...)` over multiple `.contains()` calls.
