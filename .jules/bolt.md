...
## 2026-03-13 - [Optimize percent_decode with fast-path and Cow]
**Learning:** Functions that return `String` for mostly-unchanged inputs cause unnecessary allocations. The `percent_decode` function in `stdlib/text.rs` unconditionally allocated a `Vec<u8>` and `String` even when the input had no URL-encoded characters.
**Action:** Refactor such functions to return `std::borrow::Cow<'_, str>`. Before doing any work, scan the byte slice (e.g., `bytes.iter().any(|&b| b == b'%' || b == b'+')`). If no decoding is needed, return `Cow::Borrowed(s)`. If decoding is needed, pre-allocate the `Vec` with the exact length of the bytes slice using `Vec::with_capacity(bytes.len())` to avoid intermediate resizing.
