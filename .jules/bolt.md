...
## 2026-04-11 - [Replace O(N) Peekable iter clone count with O(1) len]
**Learning:** In the WFL parser, `self.tokens` is a `Peekable` over a `std::slice::Iter`. The parser was using `self.tokens.clone().count()` inside `while` loops to check the remaining token count. While `Peekable` wraps the iterator, calling `.clone().count()` iterates over all remaining items on the cloned iterator, turning it into an $O(N)$ operation. This makes the parser $O(N^2)$ for long inputs. Since the underlying `Iter` implements `ExactSizeIterator`, `Peekable` also implements it, meaning we can just use `self.tokens.len()` for an $O(1)$ length check.
**Action:** In Rust, calling `.clone().count()` on a `Peekable` iterator is an O(N) operation because it consumes the cloned iterator. If the underlying iterator implements `ExactSizeIterator` (like `std::slice::Iter`), use `.len()` directly to achieve an O(1) length check, preventing O(N^2) complexity in loops.
