...
use tower_lsp::lsp_types::{Position, HoverParams, TextDocumentIdentifier, TextDocumentPositionParams, Url};
...
fn extract_hover_info_at_position(document_text: &str, line: u32, character: u32) -> Option<String> {
...
fn find_symbol_at_position(program: &Program, line: u32, character: u32) -> Option<SymbolInfo> {
    ...
    for statement in &program.statements {
        match statement {
            ...
            Statement::ActionDefinition { name, parameters, .. } => {
...
fn format_hover_info(symbol_info: &SymbolInfo) -> String {
    match symbol_info {
        ...
        SymbolInfo::StdlibFunction { name, description, signature } => {
...
"#;
    ...
    assert!(hover_info.is_some(), "Should provide hover info for variables");
...
"#;
    ...
    assert!(hover_info.is_some(), "Should provide hover info for functions");
...
"#;
    ...
    for (func_name, description) in &expected_stdlib_functions {
        ...
        assert!(!description.is_empty(), "Function description should not be empty");
        
        // Create expected hover format
        let expected_hover = format!("**WFL Standard Library**\n\n`{}`\n\n{}", func_name, description);
        assert!(expected_hover.contains("WFL Standard Library"), "Should identify as stdlib function");
    }
...
"#;
    ...
    for (keyword, description) in &wfl_keywords {
        ...
        assert!(!description.is_empty(), "Keyword description should not be empty");
        
        // Create expected hover format
        let expected_hover = format!("**WFL Keyword:** `{}`\n\n{}", keyword, description);
        assert!(expected_hover.contains("WFL Keyword"), "Should identify as WFL keyword");
    }
...
"#;
    ...
    match parser.parse() {
        Ok(program) => {
            ...
            assert!(!program.statements.is_empty(), "Should have parsed statements for hover analysis");
        }
        Err(errors) => {
...
"#;
    ...
    match parser.parse() {
        Ok(program) => {
            ...
            assert!(!program.statements.is_empty(), "Should parse complex expressions");
        ...
        Err(errors) => {
            println!("Parse errors (may be expected for complex syntax): {:?}", errors);
