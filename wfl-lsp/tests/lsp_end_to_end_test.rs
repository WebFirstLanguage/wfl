...
async fn test_lsp_with_basic_syntax_program() {
    ...
    let document_text = fs::read_to_string(test_program_path)
        .expect("Should be able to read test program");
    
    let mut diagnostic_reporter = DiagnosticReporter::new();
    let file_id = diagnostic_reporter.add_file("basic_syntax_comprehensive.wfl", document_text.clone());
    
    // Test lexing
    let tokens = lex_wfl_with_positions(&document_text);
    assert!(!tokens.is_empty(), "Should produce tokens for real WFL program");
    
    // Test parsing
    let mut parser = Parser::new(&tokens);
    match parser.parse() {
        Ok(program) => {
            assert!(!program.statements.is_empty(), "Should parse statements from real WFL program");
...
async fn test_lsp_with_error_examples() {
    ...
    if parse_error_path.exists() {
        ...
        match parser.parse() {
            ...
            Err(errors) => {
                assert!(!errors.is_empty(), "Should have parse errors for parse error example");
                
                // Test diagnostic conversion
                for error in &errors {
                    let wfl_diag = diagnostic_reporter.convert_parse_error(file_id, error);
                    assert!(!wfl_diag.message.is_empty(), "Diagnostic should have message");
                    assert!(!wfl_diag.labels.is_empty(), "Diagnostic should have location");
                }
...
async fn test_lsp_with_multiple_test_programs() {
    ...
    for test_file in &test_files {
        ...
        let tokens = lex_wfl_with_positions(&document_text);
        assert!(!tokens.is_empty(), "Should produce tokens for {}", test_file);
        ...
        match parser.parse() {
            Ok(program) => {
                ...
                match (analysis_result, type_result) {
                    ...
                    (analysis_res, type_res) => {
                        println!("Analysis/type checking issues for {} (may be expected)", test_file);
                        
                        // Test diagnostic conversion for any errors
                        if let Err(errors) = analysis_res {
                            for error in &errors {
                                let _diag = diagnostic_reporter.convert_semantic_error(file_id, error);
                            }
                        }
            ...
            Err(errors) => {
                println!("Parse errors for {} (may be expected): {} errors", test_file, errors.len());
    ...
    println!("LSP workflow test completed: {}/{} programs processed successfully", 
             successful_programs, total_programs);
    
    // We consider the test successful if we processed at least one program
    assert!(total_programs > 0, "Should have found at least one test program");
}
...
"#;
    ...
    assert!(!tokens.is_empty(), "Should produce tokens for large program");
    ...
    match parse_result {
        Ok(program) => {
            println!("Successfully parsed large program with {} statements", program.statements.len());
            ...
            assert!(total_time.as_millis() < 1000, "Total processing should be under 1 second");
            assert!(lex_time.as_millis() < 100, "Lexing should be under 100ms");
            assert!(parse_time.as_millis() < 500, "Parsing should be under 500ms");
        }
        Err(errors) => {
            ...
            println!("Diagnostic conversion took: {:?} for {} errors", diag_time, errors.len());
        }
    }
}
