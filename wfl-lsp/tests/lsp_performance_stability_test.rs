...
fn generate_large_wfl_program(num_variables: usize, num_functions: usize, num_statements: usize) -> String {
...
async fn test_lsp_performance_with_large_files() {
    ...
    for (num_vars, num_funcs, num_stmts) in &test_sizes {
        ...
        println!("Testing LSP performance with program size: {} bytes ({} vars, {} funcs, {} stmts)", 
        ...
        println!("  Lexing: {:?}, Parsing: {:?}, Analysis: {:?}, Type checking: {:?}", 
                lexing_time, parsing_time, analysis_time, type_checking_time);
        
        // Performance assertions
        assert!(lexing_time.as_millis() < 1000, 
               "Lexing should be under 1s for {} byte program", program_size);
        assert!(parsing_time.as_millis() < 2000, 
               "Parsing should be under 2s for {} byte program", program_size);
        
        if analysis_time.as_millis() > 0 {
            assert!(analysis_time.as_millis() < 3000, 
                   "Analysis should be under 3s for {} byte program", program_size);
        }
        
        if type_checking_time.as_millis() > 0 {
            assert!(type_checking_time.as_millis() < 3000, 
                   "Type checking should be under 3s for {} byte program", program_size);
        }
    }
}
    ...
    for depth in &nesting_depths {
        ...
        if *depth <= 50 {
            assert!(total_time.as_millis() < 1000, 
                   "Deep nesting (depth {}) should parse under 1s", depth);
        } else {
            // Very deep nesting might be slower but should still complete
            assert!(total_time.as_millis() < 5000, 
                   "Very deep nesting (depth {}) should parse under 5s", depth);
        }
...
async fn test_lsp_concurrent_operations() {
    ...
    for num_concurrent in &concurrency_levels {
        ...
        println!("  {} concurrent operations took: {:?}", num_concurrent, concurrent_time);
        ...
        assert!(concurrent_time < expected_max_time, 
               "Concurrent operations ({}) should complete within reasonable time", num_concurrent);
        
        // Concurrency should not be significantly slower than sequential for small numbers
        if *num_concurrent <= 5 {
            assert!(concurrent_time.as_millis() < 2000, 
                   "Small concurrent operations ({}) should be fast", num_concurrent);
        }
    }
}
...
async fn test_lsp_repeated_operations_performance() {
    ...
    for num_operations in &operation_counts {
        ...
        println!("  {} repeated operations took: {:?}", num_operations, repeated_time);
        ...
        assert!(avg_time_per_op < 100, 
               "Average time per operation should be under 100ms, got {}ms", avg_time_per_op);
        
        // Total time should scale reasonably
        assert!(repeated_time.as_millis() < 10000,
               "Total time for {} operations should be under 10s", num_operations);
    }
}
...
async fn test_lsp_error_recovery() {
    ...
    let error_scenarios = [
        ...
        ("Mixed valid/invalid", "store x as 5\nthis is invalid\ndisplay x"),
        ("Unmatched brackets", "store x as [1, 2, 3\ndisplay x"),
        ("Infinite loop potential", "count from i as 1 to 1000000\n    display i\nend count"),
        ("Very long line", &"a".repeat(10000)),
        ("Unicode characters", "store 变量 as \"测试\"\ndisplay 变量"),
        ("Special characters", "store x as \"!@#$%^&*()_+-={}[]|\\:;'<>?,./\""),
    ];
    ...
    for (scenario_name, document_text) in &error_scenarios {
        ...
        assert!(recovery_time.as_millis() < 1000,
               "Error recovery for '{}' should be fast", scenario_name);

        // Should not crash (we got here, so it didn't crash)
        match parse_result {
            Ok(_) => println!("  '{}' parsed successfully", scenario_name),
            Err(errors) => println!("  '{}' generated {} parse errors (expected)", scenario_name, errors.len()),
        }
        ...
        println!("  Error recovery for '{}' completed in {:?}", scenario_name, recovery_time);
    }
}
...
async fn test_lsp_stability_with_malformed_input() {
    ...
    for (i, malformed_input) in malformed_inputs.iter().enumerate() {
        println!("Testing malformed input {}: {:?}", i + 1, malformed_input.chars().take(50).collect::<String>());
        ...
        assert!(processing_time.as_millis() < 500,
               "Malformed input {} should be processed quickly", i + 1);

        // Should handle gracefully (not crash)
        match parse_result {
            Ok(_) => println!("  Malformed input {} parsed unexpectedly", i + 1),
            Err(errors) => println!("  Malformed input {} generated {} errors (expected)", i + 1, errors.len()),
        }
    }
}
...
async fn test_lsp_performance_regression() {
    ...
    for iteration in 0..num_iterations {
        ...
        println!("Performance regression test iteration {}: {:?}", iteration + 1, iteration_time);
    }
    ...
    assert!(avg_time.as_millis() < 1000,
           "Average LSP operation time should be under 1s, got {:?}", avg_time);
