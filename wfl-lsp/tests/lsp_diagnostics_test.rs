...
fn convert_wfl_diagnostic_to_lsp(
    ...
    let range = if let Some((span, _)) = wfl_diag.labels.first() {
        if let Some((start_line, start_character)) = diagnostic_reporter.offset_to_line_col(file_id, span.start) {
...
async fn test_lsp_diagnostic_conversion_for_parse_errors() {
    ...
    match parser.parse() {
        ...
        Err(errors) => {
            ...
            let lsp_diag = convert_wfl_diagnostic_to_lsp(&wfl_diag, &mut diagnostic_reporter, file_id);
            
            // Validate LSP diagnostic format
            assert_eq!(lsp_diag.severity, Some(DiagnosticSeverity::ERROR));
            assert_eq!(lsp_diag.source, Some("wfl".to_string()));
            assert!(!lsp_diag.message.is_empty(), "Diagnostic should have a message");
            
            // Validate range (u32 values are always >= 0, so just check they exist)
            assert!(lsp_diag.range.start.line < 1000, "Start line should be reasonable");
            assert!(lsp_diag.range.start.character < 1000, "Start character should be reasonable");
            assert!(lsp_diag.range.end.line >= lsp_diag.range.start.line, "End line should be >= start line");
        }
    }
}
...
async fn test_lsp_diagnostic_position_accuracy() {
    ...
    match parser.parse() {
        ...
        Err(errors) => {
            ...
            assert!(lsp_diag.range.start.line <= 1, "Error should be on line 1 or 2 (0-indexed)");
            
            // Should have valid character positions
            assert!(lsp_diag.range.start.character < 1000, "Start character should be reasonable");
            assert!(lsp_diag.range.end.character >= lsp_diag.range.start.character, "End character should be >= start character");
        }
    }
}
...
async fn test_lsp_diagnostic_multiple_errors() {
    ...
    match parser.parse() {
        ...
        Err(errors) => {
            ...
            assert!(lsp_diagnostics.len() >= 2, "Should have multiple LSP diagnostics");
            
            // All should be error severity
            for diag in &lsp_diagnostics {
                assert_eq!(diag.severity, Some(DiagnosticSeverity::ERROR));
                assert_eq!(diag.source, Some("wfl".to_string()));
                assert!(!diag.message.is_empty(), "Each diagnostic should have a message");
            }
        }
    }
}
...
async fn test_lsp_diagnostic_empty_document() {
    ...
    match parser.parse() {
        Ok(program) => {
            ...
            assert!(program.statements.is_empty(), "Empty program should have no statements");
            
            // Should not produce any diagnostics
            let mut analyzer = Analyzer::new();
            let analysis_result = analyzer.analyze(&program);
            assert!(analysis_result.is_ok(), "Empty program should analyze successfully");
            
            let mut type_checker = TypeChecker::new();
            let type_result = type_checker.check_types(&program);
            assert!(type_result.is_ok(), "Empty program should type check successfully");
        }
        Err(_) => {
