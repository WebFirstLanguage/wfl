...
fn simulate_hover_request(document_text: &str, position: Position) -> bool {
    ...
    chars.get(char_pos).is_some_and(|c| c.is_alphabetic())
}
...
async fn test_lsp_workflow_with_basic_syntax_program() {
    ...
    println!(
        "LSP Workflow Result for basic_syntax_comprehensive.wfl: {:?}",
        result
    );

    assert!(
        result.lexing_success,
        "Lexing should succeed for basic syntax program"
    );

    // Completion and hover should be available for well-formed programs
    if result.parsing_success {
        assert!(
            result.completion_available,
            "Completion should be available for parsed programs"
        );
        assert!(
            result.hover_available,
            "Hover should be available for parsed programs"
        );
    } else {
...
async fn test_lsp_workflow_with_stdlib_program() {
    ...
    println!(
        "LSP Workflow Result for stdlib_comprehensive.wfl: {:?}",
        result
    );

    assert!(
        result.lexing_success,
        "Lexing should succeed for stdlib program"
    );
    ...
    if result.parsing_success {
        assert!(
            result.completion_available,
            "Completion should be available for parsed stdlib programs"
        );
        assert!(
            result.hover_available,
            "Hover should be available for parsed stdlib programs"
        );
        println!("Stdlib program LSP validation completed successfully");
    } else {
        println!(
            "Note: Stdlib program has parsing issues - may use advanced syntax not fully implemented"
        );
    }
}
...
async fn test_lsp_workflow_with_containers_program() {
    ...
    println!(
        "LSP Workflow Result for containers_comprehensive.wfl: {:?}",
        result
    );

    assert!(
        result.lexing_success,
        "Lexing should succeed for containers program"
    );
    assert!(
        result.completion_available,
        "Completion should be available for container syntax"
    );
    assert!(
        result.hover_available,
        "Hover should be available for container elements"
    );
...
async fn test_lsp_workflow_with_error_examples() {
    ...
    for error_file in &error_files {
        ...
        assert!(
            result.lexing_success,
            "Lexing should succeed even for error examples"
        );
        ...
        if result.parsing_success {
            assert!(
                result.completion_available,
                "Completion should be available for parsed error examples"
            );
            assert!(
                result.hover_available,
                "Hover should be available for parsed error examples"
            );
        } else {
            println!(
                "Note: {} failed to parse, completion/hover may be limited",
                error_file
            );
        }
        ...
        if *error_file != "runtime_error.wfl" && *error_file != "type_error.wfl" {
            ...
            if result.parsing_success && !result.has_expected_errors() {
                println!(
                    "Note: {} parsed successfully but didn't generate expected errors - may be a valid program",
                    error_file
                );
            }
        }
...
async fn test_lsp_workflow_with_multiple_comprehensive_programs() {
    ...
    for program_file in &comprehensive_programs {
        ...
        if result.parsing_success {
            assert!(
                result.completion_available,
                "Completion should be available for parsed {}",
                program_file
            );
            assert!(
                result.hover_available,
                "Hover should be available for parsed {}",
                program_file
            );
        } else {
            println!(
                "Note: {} has parsing issues, completion/hover may be limited",
                program_file
            );
        }
    ...
    println!(
        "LSP Workflow Summary: {}/{} programs fully successful",
    ...
    assert!(
        success_rate >= 0.4,
        "LSP workflow success rate should be at least 40%, got {:.1}%",
        success_rate * 100.0
    );
}
...
async fn test_lsp_performance_with_large_programs() {
    ...
    for program_file in &large_programs {
        ...
        println!(
            "LSP Workflow for {} took: {:?}",
            program_file, workflow_time
        );

        // Performance assertion - LSP workflow should be reasonably fast
        assert!(
            workflow_time.as_millis() < 500,
            "LSP workflow for {} should be under 500ms, took {:?}",
            program_file,
            workflow_time
        );
        ...
        assert!(
            result.lexing_success,
            "Lexing should succeed for large program {}",
            program_file
        );
        ...
        if result.parsing_success {
            assert!(
                result.completion_available,
                "Completion should be available for parsed large program {}",
                program_file
            );
            assert!(
                result.hover_available,
                "Hover should be available for parsed large program {}",
                program_file
            );
        } else {
            println!(
                "Note: Large program {} has parsing issues, completion/hover may be limited",
                program_file
            );
        }
    }
...
async fn test_lsp_workflow_with_real_world_scenarios() {
    ...
    for program_file in &real_world_programs {
        ...
        println!(
            "LSP Workflow Result for real-world scenario {}: {:?}",
            program_file, result
        );
        ...
        assert!(
            result.lexing_success,
            "Lexing should succeed for real-world program {}",
            program_file
        );
        ...
        if result.parsing_success {
            assert!(
                result.completion_available,
                "Completion should be available for parsed real-world program {}",
                program_file
            );
            assert!(
                result.hover_available,
                "Hover should be available for parsed real-world program {}",
                program_file
            );
        } else {
            println!(
                "Note: Real-world program {} has parsing issues, completion/hover may be limited",
                program_file
            );
        }
        ...
        if !result.parsing_success && !result.analysis_success {
            println!(
                "Note: {} may use advanced features not fully implemented yet",
                program_file
            );
        }
    }
