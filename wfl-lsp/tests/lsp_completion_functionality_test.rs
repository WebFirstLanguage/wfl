    ...
    match parser.parse() {
        Ok(program) => {
            ...
            assert!(
                ...
                "Should collect 'username' variable. Found: {:?}",
                labels
            );
            assert!(
                labels.contains(&"age".to_string()),
                "Should collect 'age' variable. Found: {:?}",
                labels
            );
            assert!(
                labels.contains(&"active".to_string()),
                "Should collect 'active' variable. Found: {:?}",
                labels
            );
            ...
            println!(
                "Successfully collected {} variables: {:?}",
                items.len(),
                labels
            );
        }
    ...
    match parser.parse() {
        Ok(program) => {
            ...
            assert!(
                ...
                "Should collect 'greet' function. Found: {:?}",
                labels
            );
            assert!(
                labels.contains(&"calculate".to_string()),
                "Should collect 'calculate' function. Found: {:?}",
                labels
            );
            assert!(
                labels.contains(&"simple".to_string()),
                "Should collect 'simple' function. Found: {:?}",
                labels
            );
            ...
            for item in &items {
                ...
                assert!(
                    item.detail.as_ref().unwrap().contains("Function:"),
                    "Function details should indicate it's a function"
                );
            }

            println!(
                "Successfully collected {} functions: {:?}",
                items.len(),
                labels
            );
        }
...
async fn test_stdlib_completion_collection() {
    ...
    assert!(
        labels.contains(&"length of".to_string()),
        "Should include 'length of' stdlib function. Found: {:?}",
        labels
    );
    assert!(
        labels.contains(&"first of".to_string()),
        "Should include 'first of' stdlib function. Found: {:?}",
        labels
    );
    assert!(
        labels.contains(&"uppercase".to_string()),
        "Should include 'uppercase' stdlib function. Found: {:?}",
        labels
    );
    assert!(
        labels.contains(&"lowercase".to_string()),
        "Should include 'lowercase' stdlib function. Found: {:?}",
        labels
    );
    assert!(
        labels.contains(&"random".to_string()),
        "Should include 'random' stdlib function. Found: {:?}",
        labels
    );
    ...
    for item in &items {
        ...
        assert!(
            item.detail.is_some(),
            "Stdlib functions should have details"
        );
        assert!(
            item.detail.as_ref().unwrap().contains("WFL stdlib"),
            "Stdlib details should indicate it's from stdlib"
        );
    }

    println!(
        "Successfully collected {} stdlib functions: {:?}",
        items.len(),
        labels
    );
}
...
async fn test_context_aware_completion() {
    ...
    let labels: Vec<String> = items.iter().map(|item| item.label.clone()).collect();
    assert!(
        labels.contains(&"is equal to".to_string()),
        "Should suggest comparison operators after 'if'. Found: {:?}",
        labels
    );
    ...
    assert!(
        labels2.contains(&"as".to_string()),
        "Should suggest 'as' keyword after 'store'. Found: {:?}",
        labels2
    );
...
"#;
    ...
    match parser.parse() {
        Ok(program) => {
            ...
            println!(
                "Context-aware completion test - this may not trigger in complete if statement"
            );

            println!(
                "Complete completion workflow successful with {} items",
                items.len()
            );
