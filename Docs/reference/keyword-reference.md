...
### "What about `secured`, `certificate`, `key`, `redirecting`, `content_type`, `transaction`, `schema`, `changes`?" → Not keywords
These words are recognized purely by position — inside `listen` / `respond` statements, or in `in transaction on db:` / `in transaction on db for schema changes:` / `end transaction` — and are **never reserved**. Use them as variable names freely. `raise_error` is a standard-library function, not a keyword. See [Marker Words That Are Not Keywords](reserved-keywords.md#marker-words-that-are-not-keywords-at-all).
