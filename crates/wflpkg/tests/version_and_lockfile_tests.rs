...
create map locked:
    name is \"http-client\"
    version is \"26.1.3\"
    hash is \"sha256:a3f8b2c9d4e5f6a7\"
    deps is []
end map
...
create map locked:
    name is \"json-parser\"
    version is \"25.12.8\"
    hash is \"sha256:b4c5d6e7f8a9b0c1\"
    deps is [\"text-utils\"]
end map
";
    ...
    assert_eq!(lock.packages[1].deps.len(), 1);
    assert_eq!(lock.packages[1].deps[0], "text-utils");
}

#[test]
fn test_lockfile_parse_empty() {
    // A well-formed lockfile carrying only the version envelope (no `locked`
    // records) resolves to an empty package set.
    let content = "\
create map wflpkg:
    grammar is \"1.0.0\"
end map
...
#[test]
fn test_lockfile_parse_rejects_comment() {
    // The frozen grammar rejects comments outright — there is no comment channel
    // that could diverge from the hashed content.
    let content = "\
create map wflpkg:
    grammar is \"1.0.0\"
end map
...
";
    ...
    assert!(result.is_err(), "comments must be rejected");
}

#[test]
fn test_lockfile_parse_rejects_missing_field() {
    // A `locked` record missing its required `hash` field is rejected.
    let content = "\
create map wflpkg:
    grammar is \"1.0.0\"
end map
...
";
    ...
    assert!(
        msg.contains("hash"),
        "expected a missing-`hash` error, got: {msg}"
    );
}

#[test]
fn test_lockfile_parse_rejects_missing_envelope() {
    // Every wflpkg file must begin with the `create map wflpkg:` version block.
    let content = "\
create map locked:
    name is \"my-pkg\"
    version is \"26.1.0\"
...
";
    ...
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("version block"),
        "expected a missing-envelope error, got: {msg}"
    );
}

#[test]
fn test_lockfile_parse_rejects_non_string_deps() {
    // `deps` must be a list of package-name strings, never bare integers.
    let content = "\
create map wflpkg:
    grammar is \"1.0.0\"
end map
...
";
    ...
    assert!(
        msg.contains("deps"),
        "expected a `deps` element-type error, got: {msg}"
    );
}
...
";
    // `26.13.1` is a syntactically well-formed version (grammar-accepted) but
    // month 13 is out of range, so the calendar-version check rejects it.
    let result = wflpkg::lockfile::parser::parse_lock_file(content);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
...
fn test_lockfile_parse_rejects_unexpected_line() {
    // A top-level line that is not a `create map ...:` record is rejected.
